...
class AnswerController extends Controller
{
    ...
    public function store(SubmitAnswerRequest $request): JsonResponse
    {
        ...
        $path = null;
        $committed = false;
        $audioDisk = $this->configuredAudioDisk();

        try {
            $audioFile = $request->file('audio_file');
            $path = $this->storeAudioFile($audioFile, $interview->id, $audioDisk);
            ...
            if (!$answer) {
                DB::rollBack();
                Storage::disk($audioDisk)->delete($path);
     ...
     */
    private function storeAudioFile(UploadedFile $file, int $interviewId, string $disk): string
    {

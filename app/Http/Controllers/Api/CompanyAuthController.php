...
private function getEmployeeInfo($user): ?array
{
    ...
    public function updateProfile(Request $request): JsonResponse
    {
        ...
        if ($request->hasFile('logo')) {
            ...
            if ($company->logo && $publicStorage->exists($company->logo)) {
                $publicStorage->delete($company->logo);
            }

            // حفظ الشعار الجديد
            $file = $request->file('logo');
            $filename = time() . '_' . preg_replace('/[^a-zA-Z0-9._-]/', '_', $file->getClientOriginalName());
            $path = $file->storeAs('companies/logos', $filename, $publicDisk);
            $company->logo = $path;
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'company_name' => $company->company_name,
                'email' => $company->email,
                'logo' => $company->logo ? $publicStorage->url($company->logo) : null,
                'industry' => $company->industry,
                'website' => $company->website,
                'description' => $company->description,
                'phone' => $company->phone,

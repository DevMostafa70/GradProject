...
class ProfileController extends Controller
{
    public function show(): JsonResponse
    {
        try {
            ...
            $publicStorage = Storage::disk((string) config('uploads.public_disk', 'public'));
            $avatarPath = $user->avatar
                ? (preg_replace('#^/?storage/#', '', $user->avatar) ?? $user->avatar)
                : null;
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'email' => $user->email,
                    'bio' => $user->bio ?? null,
                    'avatar' => $avatarPath ? $publicStorage->url($avatarPath) : null,
    ...
    public function uploadAvatar(Request $request): JsonResponse
{
    try {
        ...
        $publicStorage = Storage::disk($publicDisk);
        ...
        if (
            $oldAvatarPath &&
            $publicStorage->exists($oldAvatarPath)
        ) {
            $publicStorage->delete($oldAvatarPath);
        }

        $path = $request->file('avatar')
            ->store('avatars', $publicDisk);

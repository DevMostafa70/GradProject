...
final class AdminDevelopmentTeamController extends Controller
{
    ...
    public function store(DevelopmentTeamMemberRequest $request): JsonResponse
    {
        ...
        $imagePath = $request->file('image')->store('development-team', $publicDisk);
        ...
        try {
            ...
            Storage::disk($publicDisk)->delete($imagePath);
            throw $exception;
        }
    ...
    public function update(
        ...
        if ($request->hasFile('image')) {
            $publicDisk = (string) config('uploads.public_disk', 'public');
            $newImagePath = $request->file('image')->store('development-team', $publicDisk);
            ...
            try {
                ...
                Storage::disk($publicDisk)->delete($newImagePath);
                throw $exception;
            }

            if ($oldImagePath) {
                Storage::disk($publicDisk)->delete($oldImagePath);
            }
        } else {
            $member->fill($data)->save();
        }
    ...
    public function destroy(DevelopmentTeamMember $member): JsonResponse
    {
        ...
        if ($imagePath) {
            Storage::disk((string) config('uploads.public_disk', 'public'))->delete($imagePath);
        }
